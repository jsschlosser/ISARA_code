...
def Run():
    ...
    * ``os``
	...
	IFN = [f for f in os.listdir(f'../ISARA_data_files/{camp_name}/Retrievals/') if (f.startswith(f'{camp_name_lower}-mrg{resolution}_{reference_platform}'))&(f.endswith('.npy'))]	
	...
	for input_filename in IFN:
		...
		output_dict = np.load(f'../ISARA_data_files/{camp_name}/Retrievals/{input_filename}',allow_pickle='TRUE')
	...
	np.save(f'../ISARA_data_files/{camp_name}/Retrievals/{camp_name_lower}-mrg{resolution}_{reference_platform}_DataRetrievals.npy', OP_Dictionary) 
